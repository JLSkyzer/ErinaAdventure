...
import fr.jlstrykerz.mcmod.procedures.ReturnToOverworldOnKeyPressedProcedure;
...
public class ReturnToOverworldKeyBinding extends MinecraftmodModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	private KeyBinding keys;
	public ReturnToOverworldKeyBinding(MinecraftmodModElements instance) {
		super(instance, 88);
	...
	public void initElements() {
		keys = new KeyBinding("key.mcreator.return_to_overworld", GLFW.GLFW_KEY_O, "key.categories.gameplay");
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					MinecraftmodMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(0, 0));
		...
		if (type == 0) {
			{
				...
				ReturnToOverworldOnKeyPressedProcedure.executeProcedure($_dependencies);
			}
		}
	}
}
