...
package fr.jlstrykerz.mcmod.world.biome;
...
import fr.jlstrykerz.mcmod.block.PlanetAlphaDirtBlock;
import fr.jlstrykerz.mcmod.MinecraftmodModElements;

import com.google.common.collect.ImmutableList;

@MinecraftmodModElements.ModElement.Tag
public class PlanetAlphaBiomeBiome extends MinecraftmodModElements.ModElement {
	public static Biome biome;
	public PlanetAlphaBiomeBiome(MinecraftmodModElements instance) {
		super(instance, 81);
	...
	private static class BiomeRegisterHandler {
		...
		public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				BiomeAmbience effects = new BiomeAmbience.Builder().setFogColor(-11193549).setWaterColor(-7896189).setWaterFogColor(-7305080)
						.withSkyColor(-11193549).withFoliageColor(-12115936).withGrassColor(-7703693).build();
				BiomeGenerationSettings.Builder biomeGenerationSettings = new BiomeGenerationSettings.Builder().withSurfaceBuilder(
						SurfaceBuilder.DEFAULT.func_242929_a(new SurfaceBuilderConfig(PlanetAlphaGrassBlock.block.getDefaultState(),
								PlanetAlphaDirtBlock.block.getDefaultState(), PlanetAlphaDirtBlock.block.getDefaultState())));
				...
				biome = new Biome.Builder().precipitation(Biome.RainType.RAIN).category(Biome.Category.EXTREME_HILLS).depth(0.1f).scale(0.9f)
						.temperature(0.5f).downfall(0.5f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("minecraftmod:planet_alpha_biome"));
			}
		}
	}
	@Override
	...
	private static class CustomLeaveVineTreeDecorator extends LeaveVineTreeDecorator {
		...
		static {
			...
			tdt.setRegistryName("planet_alpha_biome_lvtd");
	...
	private static class CustomTrunkVineTreeDecorator extends TrunkVineTreeDecorator {
		...
		static {
			codec = com.mojang.serialization.Codec.unit(() -> instance);
			tdt = new TreeDecoratorType(codec);
			tdt.setRegistryName("planet_alpha_biome_tvtd");
			ForgeRegistries.TREE_DECORATOR_TYPES.register(tdt);
		}
		@Override
		protected TreeDecoratorType<?> func_230380_a_() {
	...
	private static class CustomCocoaTreeDecorator extends CocoaTreeDecorator {
		...
		static {
			codec = com.mojang.serialization.Codec.unit(() -> instance);
			tdt = new TreeDecoratorType(codec);
			tdt.setRegistryName("planet_alpha_biome_ctd");
			ForgeRegistries.TREE_DECORATOR_TYPES.register(tdt);
		}
